...
const Transaction = () => {
  ...
  const[confirmeAddress, setConfirmeAddress]=useState(null)
  ...
  const confirmAddress = () => {
    setConfirmeAddress(account);
    console.log(confirmAddress)
  };

  const setDestinationWallet = async () => {
    try {
      if (!contract) {
      ...
      if (account){
        await contract.setDestinationWallet(account);
        await contract.destinationWallet();
        console.log("Destination wallet set");

      }
      else {
        console.error("please add a valid wallet address")
      }
  ...
  const sendEtherToDestination = async () => {
    try {
      if (!contract) {
        console.error("Contract not initialized");
        return;
      ...
      const transaction = await contract.sendEtherToDestination({value: ethers.utils.parseEther(amount),});
  ...
  return (
    <div className="bg-customBlack min-h-screen flex flex-col justify-start items-center">
      ...
      >
        ...
        <div>
          <span className="font-medium">Info alert!</span> Right now, only GETH transactions are supported.
        </div>
      </div>

      <div className="flex justify-center bg-formMain rounded-lg p-5 mt-20">
        <div className="flex flex-col justify-center items-center">
          <label htmlFor="account-input" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
            Account
          </label>
          <input
            type="text"
          ...
          <div>
            <button
              onClick={sendEtherToDestination}
